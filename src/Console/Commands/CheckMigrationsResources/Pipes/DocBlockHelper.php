...
class DocBlockHelper
{
     ...
     */
    public function addPropertiesToDocBlock(
        Class_ $class,
        string $code,
        array $newProperties,
        ...
        if ($existingDoc) {
            return $this->updateExistingDocBlock(
                $code,
                $existingDoc->getText(),
                $newProperties,
                $checkDuplicates,
     ...
     * @param  array<string>  $newProperties
     */
    private function updateExistingDocBlock(
        string $code,
        string $docText,
        array $newProperties,
        ...
        if ($checkDuplicates) {
            $newProperties = $this->filterDuplicateProperties(
                $docLines,
                $newProperties,
            );
        }
        ...
        for ($i = count($docLines) - 1; $i >= 0; $i--) {
            if (!str_contains($docLines[$i], '@property')) {
                continue;
            }
     ...
     * @param  array<string>  $newProperties
     */
    private function createNewDocBlock(
        string $code,
        Class_ $class,
        array $newProperties,
    ): string {
     ...
     * @return array<string>
     */
    private function filterDuplicateProperties(
        array $docLines,
        array $newProperties,
    ): array {
        $existingProperties = [];
        ...
        return array_filter($newProperties, function ($prop) use (
            $existingProperties,
        ) {
            if (preg_match(
                '/@property(?:-read)?\s+(.+?)\s+\$(.+)/',
                ...
                return !in_array($matches[2], $existingProperties, true);
            }
